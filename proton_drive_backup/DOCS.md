...
This add-on automatically backs up your Home Assistant instance to
...
1. On a schedule (`backup_interval_hours`), the add-on calls the Supervisor
...
Backups created by this add-on are named `Proton Drive Backup <timestamp>`.
...
| `backups_in_ha`         | How many add-on backups to keep locally in Home Assistant.                                                                                                           |
...
The add-on then stores the resulting session (not the code) and refreshes it
...
- The add-on never stores your TOTP secret/seed — only a 6-digit code you type
...
- Because login can require a manual code, fully unattended first-time startup
  ...
  add-on (or after a session expiry) to enter the code.
...
- **Enter a two-factor code** — when your account has 2FA enabled and the
  add-on needs to (re)connect (see "Two-factor authentication" above).
...
- Backups are end-to-end encrypted client-side with your Proton keys (via the
  ...
  Assistant.
- The add-on never stores your TOTP/2FA secret or any one-time code — for 2FA
...
- Your credentials are stored **unencrypted** on your HA host. Add-on options
  ...
  password) is written in plaintext to the add-on's `/data`. Anyone with
  file-level access to your host, or to an unencrypted backup of it, can read
  them. This is how all HA add-on secrets work, but you should know it.
...
This is a **third-party, community add-on**, **not** affiliated with, endorsed
by, or supported by Proton AG, and it relies on a pre-release (alpha) build of
the Proton Drive SDK. Only install add-ons you trust.
...
update the add-on to a version that supports the new model.
