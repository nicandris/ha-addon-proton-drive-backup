...
`x-pm-appversion` is `external-drive-ha_addon_proton_drive_backup@0.1.0-alpha` (required by the SDK README for third-party clients). The `{name}` must identify *this* third-party project — not `home_assistant` (impersonates the HA project) and not anything implying Proton. Must not spoof first-party apps.
