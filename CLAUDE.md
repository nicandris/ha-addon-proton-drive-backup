...
A **Home Assistant add-on** (not a custom integration) that **mirrors Home Assistant's own backups to Proton Drive** (like the Google Drive backup add-on). It does **not** create backups — it uploads whatever backups already exist in HA (automatic + manual). It is a standalone Node.js app shipped as a Docker container managed by the HA Supervisor. There is no companion Python integration.

All Proton Drive operations — and authentication — go through Proton's official first-party [`proton-drive`](https://proton.me/support/proton-drive-cli) CLI (MIT-licensed), which the add-on shells out to (`src/protonCli.mjs`). The add-on never handles Proton credentials: `auth login` prints a browser sign-in URL that the Web UI surfaces as a link; the CLI owns 2FA/passwords and persists its own session under `/data`. Remote backups are identified by **filename** (the CLI has no metadata API): `<sanitizedName> (<slug>).tar`, where `slug` is the HA backup's stable id used for dedup; retention sorts by the Proton entry's date. There is **no build/bundle step** — Node runs `src/` directly; the Dockerfile downloads the pinned CLI binary per arch with SHA-256 verification.
...
Single Node process. `src/main.mjs` wires it together: start the ingress server → initial `runSync()` after 5s → optional `setInterval` scheduler (the interval just re-checks HA for new backups to upload). No crypto/login setup — the CLI owns auth.
...
- **`orchestrator.mjs`** — `runSync(force)` (upload HA backups missing from Proton → prune Proton; **never creates backups**), `pruneHALocalNow()` (manual HA clean-up), `restoreToHA()`, `deleteProtonBackup()`, `listProtonBackups()`, `ensureSession()`, `getStatus()`/`setActivity()`. Upload/retention decisions are **pure, unit-tested functions**: `selectToUpload` (all HA backups whose slug isn't in Proton), and **two-bucket** retention split by `isAutomaticBackup(name)` (`/^Automatic backup/i` — automatic vs app; works on HA names AND Proton filenames): `selectProtonToPrune(entries, keepAutomatic, keepApp)` (per bucket, by date; keep<=0 = keep all → an app burst can't evict automatic) and `selectHALocalToPrune(haBackups, protonSlugs, keepAutomatic, keepApp)` — **SAFETY: never returns a slug not confirmed present in Proton, in either bucket**, so local clean-up can't delete an un-mirrored backup. `getConfig()` returns the effective config for the UI (password only as `backupPasswordSet` boolean). Helpers: `sanitizeName`, `remoteNameFor`, `slugFromRemoteName`, `isOurRemoteFile` (= `.tar`). A module-level `syncing` guard prevents overlapping syncs. Archive staging is `tmpDir()` = `STAGING_DIR` or the container tmp dir — **outside `/data`** (HA full-backups include `/data`, so a temp `.tar` there gets swallowed). A download `404` (backup HA lists but no longer serves) is **skipped** via `isNotFoundError`, not a hard error. `setActivity`/`getStatus` expose live `syncing`/`activity`/`progress` for the UI. Resilient: per-backup errors are caught into `lastError`; `runSync` never throws out.
- **`supervisor.mjs`** — HA Supervisor client (`http://supervisor`, `SUPERVISOR_TOKEN`). List/download/upload/delete/restore backups + `hostInfo` (**no `createBackup` — the add-on doesn't create backups**). Granted `hassio_role: manager` in `config.yaml`. `downloadBackup` attaches the HTTP `status` to its error so a `404` can be skipped.
- **`ingress.mjs`** — unauthenticated `node:http` server (HA ingress provides auth). Serves a self-contained HTML page + JSON endpoints (`/api/status`, `/api/connect`, `/api/disconnect`, `/api/sync-now`, `/api/prune-ha`, `/api/restore`, `/api/delete`, `/api/log-level`). `/api/connect` starts `cli.login` in the background and surfaces the sign-in URL via `/api/status`; `/api/sync-now` fires `runSync(true)`; `/api/prune-ha` awaits `pruneHALocalNow()` and returns `{deleted, skippedNotInProton}`. `/api/status` also carries live `syncing`/`activity`/`progress` + `settings` (`orchestrator.getConfig()`; password only as `backupPasswordSet`) + per-bucket mirror-model `stats` (`haAutomaticCount`/`haAppCount`, `protonAutomaticCount`/`protonAppCount`). The page puts the status + statistics + **settings** cards in a responsive 2-column `.grid` (1 column ≤720px, dark mode preserved), renders a connection badge plus an animated **Syncing…** badge/progress bar, a **Sync now** button (disabled/relabelled while syncing) and a **Clean up local backups** button (disabled when **both** `keep_automatic_in_ha` and `keep_app_in_ha`=0).
