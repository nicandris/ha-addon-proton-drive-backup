...
- A Proton account. If the account uses two-factor authentication, you enter a
  one-time 6-digit code in the app's web UI the first time it connects (and
...
6. **Start** the app. Optionally enable "Start on boot" and "Watchdog".
...
| `backups_in_ha`         | int      | `4`                      | Number of app backups to retain locally in Home Assistant.                                                        |
...
**Open Web UI** on the app page) where you can view status (last/next backup,
...
authenticator app and click **Connect**. The app stores the resulting
...
the app does authentication itself:

1. **SRP login (automatic).** Using your configured email and password, the
   app runs Proton's SRP handshake (`/auth/v4/info` → `/auth/v4`) and
...
2. **Two-factor gate (only if 2FA is enabled).** The session starts with
   limited scope. The app pauses and the web UI shows a code box; you enter a
   ...
   seed is ever stored.
3. **Key unlock.** The app fetches your key salts, derives the key password
...
4. **Session persistence & refresh.** The session (tokens + derived key
   password, but **not** your password, code, or seed) is saved to the app's
   local data. Expired access tokens are refreshed automatically and the rotated
   tokens re-saved, so the app keeps running across restarts. You're only
...
trusting the app with your Proton account.
...
- **Minimal secrets at rest.** The app never stores your TOTP/2FA secret or
...
- **Your credentials are stored unencrypted on your HA host.** App options
  ...
  (tokens and the derived key password) is written in plaintext to the app's
  ...
  **all** Home Assistant app secrets work; it is not unique to this app,
...
(alpha) build of the Proton Drive SDK. Install only if you trust this app.
...
the app to a version that supports the new crypto model.
